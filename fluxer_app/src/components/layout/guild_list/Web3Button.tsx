...
export const Web3Button = observer(() => {
	const {t} = useLingui();
	...
	const handleClick = () => {
		ModalActionCreators.push(modal(() => <Web3Modal />));
	};

	return (
		<div className={guildStyles.addGuildButton}>
			<Tooltip position="right" size="large" text={() => t`Web3 & Identity`}>
				<FocusRing offset={-2} focusTarget={buttonRef} ringTarget={iconRef}>
					<button
						type="button"
						aria-label={t`Web3 & Identity`}
					...
					>
						...
						>
							<CurrencyCircleDollarIcon weight="bold" className={styles.iconText} />
						</motion.div>
					</button>
				</FocusRing>
			</Tooltip>
