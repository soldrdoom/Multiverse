...
| `BOT_NOT_IN_CHANNEL` | This bot is not present in this channel |
...
| `INVALID_INTERACTION_OPTION` | Invalid interaction option |
...
| `SOLANA_WALLET_ALREADY_LINKED` | This wallet is already linked to another account |
| `SOLANA_WALLET_ALREADY_OWNED` | Your account already has a linked wallet |
| `SOLANA_WALLET_VERIFICATION_FAILED` | Could not verify ownership of this wallet |
| `SUDO_MODE_REQUIRED` | Sudo mode is required for this action |
| `TAG_ALREADY_TAKEN` | This tag is already taken |
| `TEAM_MEMBER_ALREADY_EXISTS` | That user is already a member of this team or has a pending invite |
| `TEAM_OWNER_CANNOT_BE_REMOVED` | The team owner cannot be removed from the team |
| `TEAM_STILL_OWNS_APPLICATIONS` | The team still owns applications |
...
| `TOO_MANY_APPLICATION_COMMANDS` | Too many application commands |
...
| `UNKNOWN_TEAM_MEMBER` | Unknown team member |
| `USER_CANNOT_JOIN_TEAM` | That user cannot be added to a team |
