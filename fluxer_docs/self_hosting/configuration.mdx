---
title: 'Configuration'
description: 'config.json reference for Multiverse.'
---
...
| federation? | [federation](#federation) | Federation configuration for connecting with other Multiverse instances. Default: `{}` |
...
| services | [services](#services) | Configuration for individual Multiverse services. |
...
| Property | Type | Description |
|----------|------|-------------|
| bluesky? | [bluesky](#bluesky) | Bluesky OAuth client configuration. Default: `{"enabled":true,"client_name":"Multiverse","client_uri":"","logo_uri":"https://multiverse.forum/web/apple-touch-icon.png","tos_uri":"https://fluxer.app/terms","policy_uri":"https://fluxer.app/privacy","keys":[]}` |
...
| rp_name? | string | Relying Party name displayed to users. Default: `Multiverse` |
...
| Property | Type | Description |
|----------|------|-------------|
| client_name? | string | Human-readable client name exposed to Bluesky. Default: `Multiverse` |
...
| logo_uri? | string | Optional logo presented during authorization. Default: `https://multiverse.forum/web/apple-touch-icon.png` |
...
```json
{
  "client_name": "Multiverse",
  "client_uri": "",
  "enabled": true,
  "keys": [],
  "logo_uri": "https://multiverse.forum/web/apple-touch-icon.png",
...
| static_cdn_domain? | string | Separate domain for static CDN assets (optional). Default: `multiverse.forum` |
...
```json
{
  ...
  "public_port": 8088,
  "public_scheme": "http",
  "static_cdn_domain": "multiverse.forum"
}
```
</Expandable>
...
Federation configuration for connecting with other Multiverse instances.

| Property | Type | Description |
|----------|------|-------------|
| enabled? | boolean | Enable federation with other Multiverse instances. Default: `false` |
...
Specific settings for this Multiverse instance.
...
| from_name? | string | Default sender name. Default: `Multiverse` |
...
| Property | Type | Description |
|----------|------|-------------|
| require_forwarded_for? | boolean | Reject API requests that arrive without a proxy-set client IP header (X-Real-IP, X-Forwarded-For, or CF-Connecting-IP when trust_cf_connecting_ip is on). Defaults to false because it must only be enabled when EVERY path to the API passes through a reverse proxy that sets one of those headers; a deployment reached directly on its port (including `devenv up`) rejects every request with 403 when this is on. The official deployment's nginx sets X-Real-IP and X-Forwarded-For on all proxied locations, so it can turn this on for defence in depth against requests that reach the app port without going through nginx. Default: `false` |
...
Configuration for the main Multiverse Server.
