...
| `4013` | `ACK_BACKPRESSURE` | Client fell too far behind acknowledging dispatched events | Yes |
| `4014` | `SESSION_REVOKED` | The credential that opened the session was revoked (e.g. a bot token was revoked or rotated); the session cannot be resumed. Do not reconnect with the same token | No |
