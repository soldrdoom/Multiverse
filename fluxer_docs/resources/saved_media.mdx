...
|-------|------|-------------|
| alt_text? | ?string | Alternative text description for accessibility |
| klipy_id? | ?[SnowflakeType](#snowflaketype) | Numeric Klipy clip ID, used to re-resolve the clip via the Klipy API when the meme is shared again |
...
| klipy_id? | ?[SnowflakeType](#snowflaketype) | Numeric Klipy clip ID if the meme was sourced from Klipy |
