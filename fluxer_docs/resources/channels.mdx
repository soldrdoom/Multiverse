...
| encrypted_content? | ?string | Base64 JSON NaCl box payload (E2EE DMs only) |
...
| nft_stickers? | ?[MessageNftStickerResponse](#messagenftstickerresponse)[] | The NFT stickers sent with the message |
...
| token_gate_address? | ?string | The mint or collection address gating this channel, if configured on this channel or inherited from its parent category. Null/absent if this channel has no gate of its own — check the parent category's value to know whether it's inheriting one. |
| token_gate_match_mode? | ?[TokenGateMatchMode](#tokengatematchmode) | How this channel/category's tokengate address is matched against a wallet's held assets, if configured on this channel or inherited from its parent category. Null/absent if this channel has no explicit choice of its own — check the parent category's value, defaulting to EXACT_ASSET if neither is set. |
| token_gate_satisfied? | ?boolean | Whether the requesting user currently satisfies this channel's effective tokengate. Only present on responses computed for a specific requesting user (not broadcast dispatch payloads); null/absent means no gate applies. |
| token_gate_visibility? | ?[TokenGateVisibility](#tokengatevisibility) | How this channel/category appears to members who don't satisfy its effective tokengate, if configured on this channel or inherited from its parent category. Null/absent if this channel has no explicit choice of its own — check the parent category's value, defaulting to LOCKED if neither is set. |
...
<a id="interactioncreaterequest"></a>
...
| Field | Type | Description |
|-------|------|-------------|
| bot_user_id | [SnowflakeType](#snowflaketype) |  |
| command_name | string | The name of the registered command to invoke |
| options? | ?[InteractionOptionsSchema](#interactionoptionsschema) | The option values to pass to the command |

<a id="interactionresponse"></a>
...
| Field | Type | Description |
|-------|------|-------------|
| application_id | [SnowflakeType](#snowflaketype) | The invoked command’s owning application |
| channel_id | [SnowflakeType](#snowflaketype) | The channel the command was invoked in |
| command | [GuildEmojiUpdateRequest](#guildemojiupdaterequest) |  |
| guild_id? | [SnowflakeType](#snowflaketype) |  |
| id | [SnowflakeType](#snowflaketype) | The unique identifier of this interaction |
| options | map&lt;string, string \| number \| boolean&gt; | The validated option values the command was invoked with |
| user | [UserPartialResponse](#userpartialresponse) | The user who invoked the command |
...
| E2EE | `16384` | Message content is end-to-end encrypted (DMs only) |
| COMPACT_ATTACHMENTS | `131072` | Display attachments in a compact format |

<a id="messagenftstickerresponse"></a>

## MessageNftStickerResponse

| Field | Type | Description |
|-------|------|-------------|
| collection? | string | The collection name of the NFT |
| compressed | boolean | Whether the NFT is a compressed cNFT |
| image_url | string | The URL of the NFT sticker image or video |
| media_type | [MessageNftStickerResponseMediaType](#messagenftstickerresponsemediatype) | The media type of the sticker: image (jpg/png/webp/svg), video (mp4), gif, or model (glb) |
| mint | string | The mint address of the NFT |
| name | string | The name of the NFT sticker |
...
<a id="tokengaterecheckresponse"></a>
...
| Field | Type | Description |
|-------|------|-------------|
| gate_address? | ?string | The effective mint/collection address that was checked, if any |
| satisfied | boolean | Whether the requesting user now satisfies the channel/category tokengate |

<a id="tokengatesetrequest"></a>
...
| Field | Type | Description |
|-------|------|-------------|
| address | string | The base58-encoded Solana mint or collection address gating this channel/category. What satisfies the gate depends on match_mode. |
| match_mode? | ?[TokenGateMatchMode](#tokengatematchmode) | Whether `address` must match a held asset exactly, or any asset belonging to the collection at that address. Defaults to EXACT_ASSET if omitted. |

<a id="tokengatevisibilitysetrequest"></a>
...
| Field | Type | Description |
|-------|------|-------------|
| visibility | integer (int32) | How this channel/category should appear to members who don't satisfy its effective tokengate. |
...
<a id="messagenftstickerresponsemediatype"></a>

## MessageNftStickerResponseMediaType

The media type of the sticker: image (jpg/png/webp/svg), video (mp4), gif, or model (glb)
